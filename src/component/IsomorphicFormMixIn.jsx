...
/**
 * Function used to return the route name.
 *
 * @param route - {String|Function} - A function that returns the route name. If specified as string, it represents
 *                route name.
 * @returns route name.
 */
...
var IsomorphicFormMixIn = {
    ...
    statics: {
        fetchData(params, cb) {
            ...
            var fetchAuxiliaryData
            if (typeof forms[formName] !== 'undefined') {
                fetchAuxiliaryData = forms[formName].FetchAuxiliaryData
            }

            var fetchTasklistData
            if (typeof forms[formName] !== 'undefined') {
                fetchTasklistData = forms[formName].FetchTasklistData
            }
            ...
            if (typeof fetchTasklistData === 'function') {
                tasks.push(function (data, callback) {
                    fetchTasklistData(params, data, callback)
                })
            }
                ...
                function (res) {
                    ...
                    if ( typeof(route) === 'string' ) {
                        ...
                        route.then(function (route) {
                            ...
                            logger.error(err)
                            redirectTo("generic-error", forceRedirect)
                            cb()
                        }).done()
                    }
    ...
    getChildContext() {
        var instanciateForm = function (form) {
            ...
            this.state.form = new form({
                ...
                tasklistData: data.getTasklistData(),
            })
        }
        ...
        var submit = function (e, form, query) {
            ...
            this.state.form.validate(form, (err, isValid) => {
                ...
                if (isValid) {
                    this.context.router.transitionTo(this.context.router.getCurrentPathname(), {}, _.extend({}, query), {
                        method: 'POST',
